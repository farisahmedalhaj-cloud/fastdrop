...
// Run the database seed automatically on boot. This is safe to run on every
// restart: db/seed.js checks for existing records by email/name before
// inserting anything, so re-running it never creates duplicates. Running it
// in-process (instead of as a separate `npm run seed` pre-deploy step) avoids
// a native-addon crash that better-sqlite3 can hit on newer Node.js versions
// when a short-lived script's process exits — a long-lived server process
// never hits that code path.
try {
  const seed = require('./db/seed');
  seed();
} catch (err) {
  console.error('Seed skipped/failed on boot:', err.message);
}
