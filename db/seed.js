...
function main() {
  ...
  if (!ownerEmail || !ownerPassword) {
    throw new Error('OWNER_EMAIL and OWNER_PASSWORD must be set before seeding. Aborting.');
  }
...
// Only run + exit immediately when invoked directly as a script
// (`node db/seed.js` / `npm run seed`). When required as a module (e.g. by
// server.js on boot) the caller controls the lifecycle and the db connection
// stays open, avoiding the native-addon crash-on-exit issue below entirely.
if (require.main === module) {
  main();
  // Explicitly close the database connection before the process exits.
  // Newer Node.js versions (22+) can crash with a native-addon assertion
  // ("RemoveEnvironmentCleanupHook ... env != nullptr") if better-sqlite3's
  // Statement/Database objects are torn down implicitly during Node's own
  // process-exit cleanup instead of being closed first. Closing explicitly
